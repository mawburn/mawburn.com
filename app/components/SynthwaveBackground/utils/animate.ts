    ...
    if (objects.wireframeObjects) {
      objects.wireframeObjects.children.forEach((object: THREE.Object3D) => {
        ...
        if (!reducedMotion) {
          ...
          if (Math.abs(object.position.x) > movement.bounds.x ||
          ...
          object.position.x = Math.cos(visibleAngle) * distanceFromCenter * -Math.sign(object.position.x)
          object.position.y = Math.sin(visibleAngle) * distanceFromCenter * -Math.sign(object.position.y)
